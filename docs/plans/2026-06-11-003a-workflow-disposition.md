...
| `pii-policy-check.yml` | PII/secret policy check on PRs | KEEP-THIN | PLANNED absorption into `ci.yml` — the PII job is NOT yet in ci.yml (its head/base SHA contexts are GitHub-only); pii-policy-check.yml remains the enforcement until ported. Same U9/U10 split as sanitise-wall — fork PRs are highest-PII-risk, the Actions copy is the retained sandbox. |
| `spec-validation.yml` | Validates `specs/issue-N-spec.md` shape on spec PRs | KEEP-THIN | PLANNED absorption into `ci.yml` — the spec-validation job is NOT yet in ci.yml; spec-validation.yml remains the enforcement until ported. Note: #1599 U2 also lists it in the deterministic dev-chain disable set — the check survives as a `ci.yml` job even after the orchestration trigger is box-owned. |
