...
| `pii-policy-check.yml` | PII/secret policy check on PRs | KEEP-THIN | Absorbed by `ci.yml` (PII job). Same U9/U10 split as sanitise-wall — fork PRs are highest-PII-risk, the Actions copy is the retained sandbox. |
| `spec-validation.yml` | Validates `specs/issue-N-spec.md` shape on spec PRs | KEEP-THIN | Absorbed by `ci.yml` (spec-validation job). Note: #1599 U2 also lists it in the deterministic dev-chain disable set — the check survives as a `ci.yml` job even after the orchestration trigger is box-owned. |
