...
> **Important — product vs. service.** A second axis, orthogonal to product-vs-meta, separates the **product** (`atvirokodosprendimai/wgmesh` — the AGPL mesh software) from the **service** (`atvirokodosprendimai/cloudroof-eu` — the managed hosting at cloudroof.eu the company monetizes). Every generated item carries a `surface:product` or `surface:service` label and is filed in the matching repo: product code → wgmesh; service code (landing, pricing, signup, analytics) → cloudroof-eu; service go-to-market that needs capital, pricing, or human outreach → the `needs-human` / Quackback decision queue. **wgmesh metrics measure product traction (stars, releases, convergence); cloudroof metrics measure service revenue (subscribers, trials).** Never file go-to-market work against wgmesh — that is the conflation the 2026-06-21 split corrected. The monetization boundary stays at the service layer per `CONSTITUTION.md` (the product is never paywalled).

- **Paid customers** — count of subscribers paying for any seeded product. Source: Polar/Stripe. Lagging.
- **Goose autonomous-ship rate** — % of `impl: Issue #N` PRs in the seeded product repo (currently wgmesh) that merge with zero `## Escalated to Human Review` comments and zero `awaiting-tests`/`awaiting-verification` reopen events. (`awaiting-tests` and `awaiting-verification` are labels defined and managed by wgmesh's `.github/workflows/impl-merged-close.yml` test-gate workflow — not labels in this meta-pipeline repo's label set.) Source: `gh pr list --repo <seed-repo> --search '"impl: Issue #" in:title is:merged'` + comment/label scan. Leading.
...
- **Active stuck issues** — open issues in the seeded product repo at `needs-human` OR in `copilot-triaging` >24h OR in retry cooldown >24h. Sources: `gh issue list` (label + age filters for the first two) + `company/pipeline-health-state.json` `retry_tracker.<n>.cooldown_until` (for the cooldown filter). Leading.
