...
The project board shows where everything is. Columns move automatically based on labels — never drag cards manually.
...
The board-sync workflow (`.github/workflows/board-sync.yml`) ships with placeholder IDs. To activate it:

### 1. Create a GitHub Project

- Go to your org/user **Projects** tab → **New project** → **Board**
- Add 6 columns: `Triage`, `Spec in Progress`, `Review Spec`, `Building`, `Review Code`, `Done`
- Link the repo: **Settings** (gear icon) → **Manage access** → add your repository

### 2. Get the Project and Field IDs

Replace `ORG` and `PROJECT_NUMBER` with your values:
...
Replace the `__PLACEHOLDER__` values in `.github/workflows/board-sync.yml` with the IDs from step 2.

### 4. Add PUSH_TOKEN Secret

The workflow needs a PAT with `repo` and `project` scopes. Add it as a repository secret named `PUSH_TOKEN`.

### 5. Enable Built-in Automations (optional)

In the project board settings, enable:
- **Item added** → set status to **Triage**
- **Item closed** → set status to **Done**
- **Pull request merged** → set status to **Done**

These are backup automations — the workflow handles label-driven moves.
