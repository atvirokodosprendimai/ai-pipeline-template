...
def _resolution_pattern(issue_number: int) -> re.Pattern[str]:
    # Must stay semantically identical to GitHubClient.has_merged_resolution_pr:
    # exact resolution-title formats only; PRs that merely MENTION the issue
    # must not resolve it (the loose-token Search API lesson, #1656). The
    # conformance suite locks this on both adapters.
    return re.compile(
...
class GiteaForge(GitHubClient):
    def __init__(
        ...
        *,
        ...
        # No new Config fields: the Gitea root is duck-typed off the config
        # object (set GITEA_URL in the environment of whatever constructs it),
