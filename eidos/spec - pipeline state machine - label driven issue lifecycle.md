...
- Validation outcome is communicated by label, not by CI status alone: a passing spec gets `approved-for-build` directly (the `spec-ready` label exists in `labels.yml` for potential manual workflows but is not used by the automated validation path); a failing spec gets `spec-needs-fix`. Both states are visible on the Kanban dashboard.
...
- When a spec PR is merged, the build phase starts automatically: the originating issue receives `building` and a build agent is assigned with instructions to read the merged spec and produce an implementation PR including tests. The build agent role is configurable — currently defaults to Copilot (`copilot-swe-agent[bot]`) via the GitHub agent assignment API, but can be swapped to Goose or any PR-opening agent. {>> spec-merged-build.yml fires on `pull_request.closed` where `merged == true` and title contains `spec:`. Issue number is extracted from the PR title via regex.}
...
**Active workflows vs templates.** Active workflows live in `.github/workflows/` and run in this repo. Workflow templates live in `.github/workflow-templates/` and are activated by forks when they configure their build agent (e.g., `goose-build.yml` for Goose). The template repo uses Copilot as both spec writer and build agent; forks may swap the build agent to Goose via the template.
...
- Agent instructions and issue titles are sanitised before publishing, per [[spec - security quality - constitution and enforcement]] (SEC-2, SEC-3).
